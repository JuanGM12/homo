...
ENV_PATH = ROOT / ".env"
DEFAULT_PATTERN = "Listado de Asistencia Actividades*.xlsx"
...
def timestamp_to_string(value: Any, fallback: Any = None) -> str:
    ...
    if text == "":
        raise ValueError("Marca temporal vacia.")
...
def date_to_string(value: Any, fallback: Any = None) -> str:
    ...
    text = clean_text(value)
    if text == "":
        raise ValueError("Fecha de actividad vacia.")
    ...
    raise ValueError(f"Fecha invalida: {text!r}")
...
def resolve_user(
    ...
    alias_target = aliases.get(normalized_name)
    if alias_target:
        matches = by_name.get(normalize_text(alias_target), [])
        if len(matches) == 1:
...
def normalize_group_item(item: str) -> str:
    ...
    mappings = {
        ...
        "victima de conflito armado": "Victima del conflicto armado",
        "victima del conflito armado": "Victima del conflicto armado",
        "victima del conflicto": "Victima del conflicto armado",
        "victima del conflicto armado": "Victima del conflicto armado",
        "victima de conflicto armado": "Victima del conflicto armado",
...
def normalize_sex(raw_value: Any) -> str | None:
    ...
    mappings = {
        ...
        "transgenero, transexual o travesti": "Transgenero, transexual o travesti",
        "transgenero transexual o travesti": "Transgenero, transexual o travesti",
    }
...
def load_existing_activity_map(
    ...
    with connection.cursor() as cur:
        cur.execute(
            """
            SELECT id, code, subregion, municipality, lugar, advisor_user_id, activity_date, tipo, actividad_tipos
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Importa masivamente listados de asistencia de actividades libres desde Excel."
    )
    parser.add_argument("--file", help="Ruta exacta del xlsx a importar.")
    parser.add_argument("--dir", default=str(Path.home() / "Downloads"), help="Carpeta donde estan los xlsx.")
    parser.add_argument("--pattern", default=DEFAULT_PATTERN, help="Patron de archivos a importar.")
    parser.add_argument("--name-aliases", help="JSON opcional con aliases de nombre para asesores.")
    parser.add_argument(
        "--skip-unresolved",
        action="store_true",
        help="Importa las filas validas aunque queden asesores o filas sin resolver.",
    )
...
def main() -> int:
    ...
    try:
        by_name = build_user_index(connection)
        ...
        with connection.cursor() as cur:
            for file_path in files:
                ...
                col_created = find_column(headers, "Marca temporal")
                col_activity_date = find_column(headers, "Fecha registro")
                col_place = find_column(headers, "Lugar")
                col_activity = find_column(headers, "Actividad")
                col_advisor = find_column(headers, "Asesor")
                col_subregion = find_column(headers, "Seleccione la subregion")
                municipality_cols = find_columns(headers, "Seleccione el municipio")
                col_full_name = find_column(headers, "Nombres y Apellidos completos")
                col_document = find_column(headers, "No. Documento de identidad")
                col_entity = find_column(headers, "Entidad/Organizacion")
                col_cargo = find_column(headers, "Cargo")
                col_phone = find_column(headers, "Telefono")
                col_email = find_column(headers, "Correo electronico")
                ...
                col_group = find_column(headers, "Grupo poblacional")
                ...
                for row_number, row in enumerate(rows, start=2):
                    ...
                    advisor_name_excel = clean_title_text(get_cell(row, col_advisor))
                    advisor_user, mode = resolve_user(advisor_name_excel, aliases, by_name)
                    if advisor_user is None:
                        unresolved.append(
                            f"{filename} fila {row_number}: no se encontro usuario para asesor "
                    ...
                    lugar = clean_title_text(get_cell(row, col_place))
                    activity_types = parse_activity_types(get_cell(row, col_activity))
                    if subregion == "" or municipality == "" or lugar == "" or activity_types == []:
                        unresolved.append(
                            f"{filename} fila {row_number}: faltan datos de actividad "
                            f"(subregion/municipio/lugar/actividad)."
                        )
                        continue
                    ...
                    if activity_key not in prepared_activities:
                        ...
                        for item in activity_types:
                            if normalize_text(item) not in existing_type_keys:
                    ...
                    if document_key in prepared_activity["attendee_docs_in_file"]:
                        ...
                        warnings.append(
                            f"{filename} fila {row_number}: documento duplicado {document_number!r} "
                            f"dentro de la misma actividad; se omitio."
                        )
                        continue
                    prepared_activity["attendee_docs_in_file"].add(document_key)

                    age = normalize_age(get_cell(row, col_age))
                    if col_age >= 0 and clean_text(get_cell(row, col_age)) != "" and age is None:
                        warnings.append(
                            f"{filename} fila {row_number}: edad invalida "
                            f"{clean_text(get_cell(row, col_age))!r}; se guardo vacia."
                        )
                ...
                for activity_key, activity in prepared_activities.items():
                    if activity["attendees"] == []:
                        warnings.append(
                            f"{filename}: actividad en {activity['municipality']} / {activity['lugar']} "
                            f"quedo sin asistentes validos y se omitio."
                        )
                        continue
                    ...
                    if existing_activity is not None:
                        ...
                        if args.commit:
                            ...
                            existing_activity_map[activity_key] = {"id": activity_id, "code": code}
